...
public interface ClientRepository extends JpaRepository<Client, Long> {
    ...
    @Query("SELECT c FROM Client c WHERE c.tenant.id = :tenantId AND REPLACE(LOWER(TRIM(c.cinPassport)), ' ', '') = REPLACE(LOWER(TRIM(:cin)), ' ', '')")
    Optional<Client> findByTenantIdAndCinPassport(@Param("tenantId") Long tenantId, @Param("cin") String cin);

    @Query("SELECT c FROM Client c WHERE c.tenant.id = :tenantId AND REPLACE(LOWER(TRIM(c.iceNumber)), ' ', '') = REPLACE(LOWER(TRIM(:ice)), ' ', '')")
    ...
    @Query("SELECT c FROM Client c WHERE REPLACE(LOWER(TRIM(c.cinPassport)), ' ', '') = REPLACE(LOWER(TRIM(:cin)), ' ', '') AND c.blacklisted = true")
    List<Client> findBlacklistedByCin(@Param("cin") String cin);

    @Query("SELECT c FROM Client c WHERE REPLACE(LOWER(TRIM(c.iceNumber)), ' ', '') = REPLACE(LOWER(TRIM(:ice)), ' ', '') AND c.blacklisted = true")
